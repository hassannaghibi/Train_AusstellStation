...
def first_line_chart(input, url, exportFile):
    ...
    if len(result) > 0:
        max_y = int(max(result)) + 1

    ax.yaxis.grid(color='gray', linestyle='-', linewidth=0.1)
    p1 = plt.bar(ind, result, width, color='yellow')

    plt.ylabel('')
    plt.xlabel('', fontweight="bold")
    plt.title('Average Piles Height', fontweight="bold")
    plt.xticks(np.arange(1, N, 1), fontsize=8)
    plt.yticks(np.arange(0, max_y, 0.5), fontsize=8)
    ...
    fig.savefig(url + "VisualizationAveragePilesHeight.png")  # save chart pic
...
def second_line_chart(input, url):
    ...
    fig.savefig(url + "VisualizationAveragePilesHeightSecond.png")  # save chart pic
...
print("start")
url = "C:/Users/Hassan/Desktop/pythonExport/VisualizationAveragePilesHeight/"
